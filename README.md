...
Build on Windows from `cmd`. Use JDK 17 in Android Studio. Do not use JDK 25 or Gradle 9. This project pins Gradle 8.9 and Android Gradle Plugin 8.7.3. The launcher label is `Cocoon RomM Shelf 0.1.2`. Bright and dark cloud themes are in Settings. Covers already stored on RomM are shown in the app. CocoonFE does not document a cover filename Cocoon scans, so a cover is not copied into the ROM folder.
